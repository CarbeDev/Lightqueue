...
- Optional `attemptTimeout` DSL setting for both queue types. The timeout is applied
  independently to every handler invocation, excludes retry backoff, and follows the
  existing retry/dead-letter path with `AttemptTimeoutException` when it expires.
