...
class PriorityQueueDsl<T> internal constructor(private val scope: CoroutineScope) {
     ...
     * when exporting metrics. Also used as the base for the per-level [QueueMetrics.name]
     * returned by [PriorityInMemoryQueue.metrics] (e.g. `"webhooks:HIGH"`).
     */
    ...
    /**
     * Test-only escape hatch: lets PriorityInMemoryQueueTest build a queue with no
    ...
    internal fun toPriorityInMemoryQueue(): PriorityInMemoryQueue<T> {
        if (!allowNoWorkers) {
